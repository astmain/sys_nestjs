...
export class AppAuthorized implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // 解析token得到user信息
    try {
      let payload = await this.jwt_service.verifyAsync(token, { secret: process.env.jwt_secret })
...
@Module({
  imports: [
    JwtModule.register({ global: true, secret: process.env.jwt_secret, signOptions: { expiresIn: process.env.jwt_time_expiresIn } }), //jwt//私钥//24小时*30天
  ],
  controllers: [],
  providers: [
    {
...
export class AppAuthorized_module {}
